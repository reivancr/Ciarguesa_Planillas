function selectCompany(name) {
  const screen = document.getElementById('company-selector-screen');
  const app = document.getElementById('app');

  if (screen) screen.classList.add('hidden');
  if (app) app.style.display = 'flex';

  log('Sesión iniciada: ' + name);
  updDash();

  // Pequeño delay para quitarlo del DOM y mejorar rendimiento si se desea
  setTimeout(() => {
...
function go(id, el) {
  ...
  if (id === 'marcas') { console.log('Iniciando Marcas...'); syncSels(); initMarcas(); }
  ...
  document.getElementById('bol-prev').innerHTML = `<div class="boleta" style="box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04); border: 1px solid var(--color-border-secondary); margin: 0 auto; background:#fff; padding:30px; border-radius:12px;">
    <div class="bol-h" style="display:flex; justify-content:space-between; border-bottom:2px solid var(--primary); padding-bottom:15px; margin-bottom:20px;">
      <div style="display:flex; align-items:center; gap:15px;">
        <img src="assets/ciarguesa_logo.png" alt="CIARGUESA" style="height:50px; width:auto;">
        <div>
          <div style="font-size:22px;font-weight:700;color:var(--primary);letter-spacing:1px; line-height:1.1;">CIARGUESA</div>
          <div style="font-size:11px;color:var(--accent);font-weight:600;text-transform:uppercase;">Constructora Industrial Arguedas S.A</div>
        </div>
      </div>
...
function getBolHTML(e, des, has, hex, ot, dias) {
  ...
  return `
    <div style="font-family: Arial, sans-serif; color: #1e293b; max-width: 600px; border: 1px solid #e2e8f0; padding: 25px; border-radius: 12px; background: #fff;">
      <div style="border-bottom: 2px solid #0F172A; padding-bottom: 15px; margin-bottom: 20px; display: flex; align-items: center; gap: 15px;">
        <img src="http://localhost:3001/assets/logo.png" alt="Logo" style="height:45px; width:auto;">
        <div>
          <h2 style="margin:0; color:#0F172A; line-height:1.1;">CIARGUESA</h2>
...
function renderMarcas() {
  ...
  tb.innerHTML = filtered.map(m => {
    ...
    return `<tr>
      ...
      <td>
        <button class="btn sm dan" onclick="delMarca('${m.id}')"><i class="ti ti-trash"></i></button>
      </td>
    </tr>`;
  }).join('');
}
